...
private:
	// Fill this in with the implementation of your doubly-linked list
	// priority queue.
	struct Node{
        string value;
        Node* next;
	...
	Cell(std::string n_value = std::string())

	Cell(std::string& n_value)
	: prev(NULL), next(NULL), value(n_value)
	}
	*/
};
